...
/** Matrix class to provide similar behavior that high level math languages like MATLAB, Scilab, R, ...
  ...
  Following operators are available: 
    ...
    Following static Linear Algebra (from JAMA) are available:
        Cholesky decomposition:,         Cholesky_L(Matrix),        Cholesky_SPD(Matrix)
...
final public double getAt(java.util.List<Integer>  rc) {
    return d[rc.get(0)][rc.get(1)];
}


// get a submatrix consisting of the rows within the rr range, e.g.
//  A = rand(8, 9);   a23 = A[2..5]
...
public void putAt(ArrayList paramList,  double value)  {
    ...
    Object orowR = paramList.get(0);
    Object ocolR = paramList.get(1);
    ...
    if (rowR instanceof  groovy.lang.IntRange && colR instanceof IntRangeWithStep) 
        ...
        putAt((groovy.lang.IntRange)rowR, (groovy.lang.IntRange) colR, value);
}
        


// assign to a submatrix with row range rr and column range cr the value value
...
Matrix  CP(Vec colToPrepend) {
 ...
 final  public void eachValue(groovy.lang.Closure c) {
  ...
  final  public void map(groovy.lang.Closure c) {
   eachValue(c);
 }
...
final  public static Matrix sin(Matrix M) {
 ...
 final  public  Matrix round() {
     ...
     for (int k=0; k<N; k++)
         for (int c=0; c<M; c++)
           this.d[k][c] = Math.round(this.d[k][c]/conv);
 ...
 final public  Matrix sqrt() {
     ...
     for (int k=0; k<N; k++)
         for (int c=0; c<M; c++)
           this.d[k][c] = Math.sqrt(this.d[k][c]/conv);
...
final     public  static double [][] Rand(int m)  {
        return  Random(m, m);
    }
...
final public Matrix power(int n, Matrix m)  {
    ...
    final public Matrix multiply( Matrix v2)  {
            return new Matrix(LinearAlgebra.times( getRef(),  v2.getRef()), false );
	}

    final public Matrix multiply( double [][] v2)  {
...
final public static double [][] Cholesky_SPD(double [][] dM) {
    CholeskyDecomposition choleskDec = LinearAlgebra.cholesky(dM);
    double [][]  choleskMat = choleskDec.getL().getArray();
    return choleskMat;
}
        
final public static Matrix Cholesky_solve(Matrix A,Matrix b) {
...
final public static double [][] Cholesky_solve(double [][] dA, double [][] db) {
...
final public static double [][] Cholesky_solve(double [][] dA, double [] dbv) {
...
final public static Object  mean(double [] v)  {
    return StatisticSample.mean(v);
}
        
final public static Object  variance(double [] v)  {
    return StatisticSample.variance(v);
}
        
final public static Object var(double [] v)  {
    return variance( v);
}

final public static Object  std(double [] v)  {
    return StatisticSample.stddeviation(v);
}
        
final public static Object   covariance(double [] v1, double [] v2)  {
    return StatisticSample.covariance(v1, v2);
}
	
final public static Object cov(double [] v1, double [] v2)  {
    return covariance( v1,  v2);
}
		
final public static Object   correlation(double [] v1, double [] v2)  {
    return StatisticSample.correlation(v1, v2);
}
	
final public static Object corr(double [] v1, double [] v2)  {
    return correlation( v1,  v2);
}
...
final public static Matrix MTJsolve(Matrix A, Matrix b) {
    return A.MTJsolve(b);
}

final public Matrix MTJsolve(Matrix B) {
...
final public EigResults  MTJeig() {  
  EigResults eigA = groovySci.math.array.JILapack.MTJEig(this.getArray());
  return eigA;
}

final public static EigResults  MTJeig(Matrix A) {  
  EigResults eigA = groovySci.math.array.JILapack.MTJEig(A.getArray());
  return eigA;
}

final public static EigResults  MTJeig(double [][] A) {  
  EigResults eigA = groovySci.math.array.JILapack.MTJEig(A);
  return eigA;
}
...
final public static Matrix EJMLinv(Matrix A) {
...
final public static double  [] []  EJMLeigD(double [][] da) {
...
// compute eigenvalues  using EJML library
final public static double [] []  EJMLeigD(Matrix a) {
    return EJMLeigD(a.getArray());
}

// compute eigenvectors  using EJML library
// returns an array where each column keeps an eigenvector
final public static double  [] []  EJMLeigV(double [][] da) {
    org.ejml.data.DenseMatrix64F dm_da = new org.ejml.data.DenseMatrix64F(da);
    org.ejml.alg.dense.decomposition.eig.SwitchingEigenDecomposition   eig_da = new org.ejml.alg.dense.decomposition.eig.SwitchingEigenDecomposition(dm_da.numRows);
    eig_da.decompose(dm_da);
    int neigs = eig_da.getNumberOfEigenvalues();
...
final public static double  [] []  EJMLeigV(Matrix da) { 
      return  EJMLeigV(da.getArray());
}
